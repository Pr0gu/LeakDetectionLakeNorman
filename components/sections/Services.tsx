...
function ServiceCard({ service, index }: { service: (typeof SERVICES)[number]; index: number }) {
  ...
  return (
    <div
      ref={ref}
      className={`group relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] p-8 shadow-sm transition-all duration-500 hover:-translate-y-1 hover:border-brand-accent/20 hover:shadow-xl hover:shadow-brand-accent/5 ${
    ...
    >
      ...
      <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-xl bg-brand-deep/5 transition-colors group-hover:bg-brand-accent/10">
        <Icon className="h-7 w-7 text-white transition-colors group-hover:text-brand-accent" aria-hidden="true" />
      </div>
...
export default function Services() {
  ...
  return (
    <section id="services" className="section-padding bg-transparent" aria-labelledby="services-heading">
      <div className="container-narrow">
        ...
        >
          ...
          <h2 id="services-heading" className="text-balance text-3xl font-extrabold text-white sm:text-4xl md:text-5xl">
